...
// function to unhide the search result
function fadeInContainers() {
    const containers = document.querySelectorAll('.container.hidden');
      ...
      setTimeout(() => {
        ...
        const fadeEffect = setInterval(() => {
          if (opacity < 1) {
            ...
            clearInterval(fadeEffect);  // Stop when opacity is 1
          }
        }, 50);  // Adjust the speed of the fade effect here
      }, index * 400);  // Delay between each container fade-in (300ms per container)
    });
  }
...
async function fuzz() {
  ...
  try {
    ...
    document.querySelector(
      ".fuzz"
    ).innerHTML = `Scanning... <button onclick="pkill(${code},'.fuzz')">Kill</button>`
    var check = false
    while (!check) {
      await sleep(5000)
...
async function nmap() {
  ...
  try {
    ...
    document.querySelector(
      ".nmap"
    ).innerHTML = `Scanning... <button onclick="pkill(${code},'.nmap')">Kill</button>`
    var check = false
    while (!check) {
      await sleep(5000)
      check = await pcheck(code)
...
async function wafscan() {
  ...
  try {
    ...
    document.querySelector(
      ".waf"
    ).innerHTML = `Scanning... <button onclick="pkill(${code},'.waf')">Kill</button>`
    var check = false
    while (!check) {
      await sleep(5000)
      check = await pcheck(code)
...
async function cmsscan() {
  ...
  try {
    ...
    document.querySelector(
      ".cms"
    ).innerHTML = `Scanning... <button onclick="pkill(${code},'.cms')">Kill</button>`
    var check = false
    while (!check) {
      await sleep(5000)
      check = await pcheck(code)
...
async function vtotal() {
  ...
  try {
    ...
    if (searchResult["virustotal"].length != 0) {
      ...
      table_scan_result += `<p><b>Last Scan:</b> ${new Date(
      ...
      if (response_data) {
        ...
        table_res += `<p><b>Last Modification Date:</b> ${new Date(
          attributes.last_modification_date * 1000
        )}</p>`
        table_res += `<p><b>Last Submission Date:</b> ${new Date(
  ...
  try {
    ...
    Virus.forEach(element => {
      element.vir_result.forEach(vtdataElement => {
        ...
        table_scan_result += "<h3>Scan Result</h3>"
        table_scan_result += `<p><b>Last Scan:</b> ${new Date(
          vtdataElement.attributes.last_analysis_date * 1000
        )}</p>`
        table_scan_result += "<table>"
        table_scan_result += "<th>Engine</th><th>Category</th><th>Result</th><th>Method</th>"
        ...
        if (last_http_response_headers) {
          ...
          table_res += `<p><b>Last Modification Date:</b> ${new Date(
            vtdataElement.attributes.last_modification_date * 1000
          )}</p>`
          table_res += `<p><b>Last Submission Date:</b> ${new Date(
            vtdataElement.attributes.last_submission_date * 1000
          )}</p>`
          table_res += "<table>"
          table_res += "<th>Header</th><th>Value</th>"
